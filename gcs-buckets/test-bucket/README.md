...
This Terraform configuration creates a simple Google Cloud Storage bucket named "test-bucket" with versioning enabled.
...
| bucket_name | Name of the GCS bucket | test-bucket |
...
1. **Upload objects**: `gsutil cp file.txt gs://test-bucket/`
2. **List versions**: `gsutil ls -a gs://test-bucket/`
3. **Access specific version**: `gsutil cp gs://test-bucket/file.txt#<generation> ./`
...
```bash
# Grant object admin access to specific bucket
gsutil iam ch user:EMAIL:objectAdmin gs://test-bucket

# Grant viewer access to specific bucket
gsutil iam ch user:EMAIL:objectViewer gs://test-bucket
```
